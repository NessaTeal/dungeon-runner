...
func perform_bonk() -> void:
	var damage := speed_component.current_speed * coefficient
	if damage > 0:
		var projectile := slingshot_projectile_script.instantiate() as SlingshotProjectile
